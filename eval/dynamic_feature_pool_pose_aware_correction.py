...
def process_probes_combined(probes_path, initial_gallery, max_pool_size=10, consistency_threshold=0.3):
    print("Processing probes with Pose Awareness, Self-Consistency Correction, and Learning from Failure...")
    ...
    for person_path, student_id in tqdm(person_folders, desc="Processing Probes"):
        ...
        for image_path in image_files:
            ...
            # --- Self-Consistency Check (Pose-Aware) ---
            student_gallery_feats = dynamic_gallery.get(student_id)
            if student_gallery_feats:
                # Find the best pose-matched feature within the claimed identity's gallery
                best_pose_gfeat_for_consistency = None
                max_pose_sim_for_consistency = -1
                for gfeat in student_gallery_feats:
                    pose_sim = pose_similarity_score(probe_pose, gfeat['pose'])
                    if pose_sim > max_pose_sim_for_consistency:
                        max_pose_sim_for_consistency = pose_sim
                        best_pose_gfeat_for_consistency = gfeat
                
                # Check consistency against that single best-pose feature
                if best_pose_gfeat_for_consistency:
                    consistency_score = np.dot(probe_feature, best_pose_gfeat_for_consistency['embedding'])
                    if consistency_score < consistency_threshold:
                        continue # Discard inconsistent probe

            # --- Pose-Aware Matching Logic ---
            scores = {}
            best_pose_features = {} 
            for gid, gfeats in dynamic_gallery.items():
                ...
                # Step 1: Find the gallery feature with the best pose similarity for this ID.
                ...
                # Step 2: Calculate score using only the feature similarity of the selected feature.
                if best_pose_gfeat:
                    scores[gid] = np.dot(probe_feature, best_pose_gfeat['embedding'])
                    best_pose_features[gid] = best_pose_gfeat # Store this for potential correction
                else:
                    scores[gid] = 0.0
...
def calculate_and_display_metrics(genuine_scores, imposter_scores, correct_ids, total_probes, initial_gallery, final_gallery):
    ...
    print(f"ASAR@FAR 1e-3: {tar:.4f}")

    # TAR @ FAR=1e-2
    far_val = 1e-2
    idx = np.where(fpr <= far_val)[0]
    tar = tpr[idx[-1]] if len(idx) > 0 else 0.0
    print(f"TAR@FAR 1e-2: {tar:.4f}")

    # TAR @ FAR=1e-1
    far_val = 1e-1
    idx = np.where(fpr <= far_val)[0]
    tar = tpr[idx[-1]] if len(idx) > 0 else 0.0
    print(f"TAR@FAR 1e-1: {tar:.4f}")
    ...
    plt.title('ROC Curve - Combined Pose-Aware Correction')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate with pose awareness and self-consistency correction.")
