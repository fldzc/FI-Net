...
def process_probes_combined(probes_path, initial_gallery, max_pool_size=10, consistency_threshold=0.3, num_fusion_features=4):
    print("Processing probes with ProxyFusion, Pose Awareness, Self-Consistency Correction, and Learning from Failure...")
    ...
    for person_path, student_id in tqdm(person_folders, desc="Processing Probes"):
        ...
        for image_path in image_files:
            ...
            # --- Pose-Aware Matching Logic with ProxyFusion---
            scores = {}
            for gid, gfeats in dynamic_gallery.items():
                ...
                # Step 1: Rank gallery features by pose similarity
                gfeats_with_pose_sim = [
                    (gfeat, pose_similarity_score(probe_pose, gfeat['pose'])) for gfeat in gfeats
                ]
                gfeats_with_pose_sim.sort(key=lambda x: x[1], reverse=True)

                # Step 2: Select top N features for fusion
                top_gfeats = [gfeat for gfeat, sim in gfeats_with_pose_sim[:num_fusion_features]]
                
                fused_gallery_feature = None
                # Step 3: Fuse features if more than one is selected and model is available
                if len(top_gfeats) > 1 and fusion_model_instance and fusion_model_instance != "failed":
                    try:
                        selected_features = np.array([f['embedding'] for f in top_gfeats])
                        ...
                        # Fallback to single best feature if fusion fails
                        fused_gallery_feature = top_gfeats[0]['embedding']
                else:
                    # Use the single best pose feature if not enough features or no model
                    if top_gfeats:
                        fused_gallery_feature = top_gfeats[0]['embedding']

                # Step 4: Calculate score
                if fused_gallery_feature is not None:
                    scores[gid] = np.dot(probe_feature, fused_gallery_feature)
                else:
                    scores[gid] = 0.0
