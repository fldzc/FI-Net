...
def process_probes(probes_path, initial_gallery_features, max_pool_size=10):
    print("Processing probes with dynamic feature pool (Learning from Failure)...")
    ...
    for person_path, student_id in tqdm(person_folders, desc="Processing Probes"):
        ...
        for image_path in image_files:
            ...
            # Match against the current dynamic gallery
            scores = {}
            for gid, gfeats in dynamic_gallery.items():
                # Use the average of features for each gallery identity
                if gfeats:
                    gallery_embeddings = np.array([f['embedding'] for f in gfeats])
                    avg_gallery_feat = np.mean(gallery_embeddings, axis=0)
                    avg_gallery_feat /= np.linalg.norm(avg_gallery_feat)
                    scores[gid] = np.dot(probe_feature, avg_gallery_feat)
                else:
                    scores[gid] = 0.0
            ...
            # If the best match is NOT the correct person, it's a "hard sample".
            # We then add this hard sample to the correct person's pool to improve it.
            if best_match_id != student_id:
                ...
                if len(student_pool) < max_pool_size:
                    ...
                    # Pool is full, find the index of a non-main feature with the lowest quality
                    ...
                    if non_main_features_with_indices:
                        # Find the index of the feature with the lowest quality
                    ...
                    # If there are no non-main features, we do nothing to protect the main ones.
            
            # For accuracy metric, check if the identification was correct.
            if best_match_id == student_id:
                correct_identifications += 1

            # Store scores for ROC analysis
...
def calculate_and_display_metrics(genuine_scores, imposter_scores, correct_identifications, total_probes, initial_gallery, final_gallery):
    ...
    accuracy = (correct_identifications / total_probes) * 100 if total_probes > 0 else 0
    print(f"\n--- Evaluation Metrics ---")
    ...
    plt.title('ROC Curve - Dynamic Feature Pool')
    ...
    return {
        ...
        "acc": float(accuracy / 100.0),  # Return as a fraction
...
def main(args):
    ...
    g, i, c, t, final_gallery = process_probes(args.probes_path, 
                                             initial_gallery,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate face recognition with a dynamic feature pool.")
    parser.add_argument('--probes_path', type=str, default='dataset/NB116_person_spatial', help='Path to probe images.')
    parser.add_argument('--gallery_path', type=str, default='dataset/images/faces_images', help='Path to gallery images.')
