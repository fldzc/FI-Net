...
def get_parser():
    parser = argparse.ArgumentParser(description="处理stream目录中所有课程的人员ID识别与组织")
    parser.add_argument(
        "--config-file",
    ...
    parser.add_argument(
        "--output-folder",
        default="dataset/NB116_person_reid",
        help="保存结果的文件夹路径",
    )
    parser.add_argument(
        "--threshold",
...
def compute_position_similarity(boxes1, boxes2, img_shape):
    ...
    for i in range(n1):
        for j in range(n2):
            ...
            # 结合IoU和中心点距离的相似度
            position_similarity[i, j] = 0.6 * iou + 0.4 * center_sim
...
def process_single_course(course_path, output_path, demo, yolo_model, args):
    ...
    for ext in img_extensions:
        image_files.extend(glob.glob(os.path.join(course_path, ext)))
    
    # 确保找到图片
    if not image_files:
        print(f"警告: 在文件夹 {course_path} 中没有找到图片")
        return
...
def main():
    ...
    # 获取stream目录下所有的日期文件夹
    stream_path = args.stream_folder
    if not os.path.exists(stream_path):
        print(f"错误: stream文件夹不存在: {stream_path}")
        return
    
    # 遍历所有日期文件夹
    date_folders = [f for f in os.listdir(stream_path) if os.path.isdir(os.path.join(stream_path, f))]
    
    if not date_folders:
        print(f"错误: 在 {stream_path} 中没有找到日期文件夹")
        return
    ...
    for date_folder in tqdm(date_folders, desc="处理日期文件夹"):
        date_path = os.path.join(stream_path, date_folder)
        
        # 获取该日期下的所有教室文件夹
        classroom_folders = [f for f in os.listdir(date_path) if os.path.isdir(os.path.join(date_path, f))]
        
        for classroom_folder in tqdm(classroom_folders, desc=f"处理{date_folder}中的教室", leave=False):
            classroom_path = os.path.join(date_path, classroom_folder)
            
            # 获取该教室下的所有课程文件夹
            class_folders = [f for f in os.listdir(classroom_path) if os.path.isdir(os.path.join(classroom_path, f))]
            
            for class_folder in class_folders:
                class_path = os.path.join(classroom_path, class_folder)
                total_courses += 1
                
                # 创建对应的输出路径
                output_class_path = os.path.join(args.output_folder, date_folder, classroom_folder, class_folder)
